...
x <- read.table("CLLCRC_SurvAgeGender_NoPreviousTreatm.txt", sep="\t", head=T, quote="\"", check.names=F, na="")
...
for (m in mir) {
	...
	l <- lapply(l, scale)
	tiff(paste(m,"Age_CLLCRC_2.tiff", sep="_"), width=11*300, height=2700, res=300, compression="lzw")
	par(oma=c(3,5,0,0),font=2)
	cex <- 2.8
	r0 <- c(10, 60, 600)
	r1 <- scale(r0)
	b <- boxplot(l, lwd=4, cex.axis=2.8, cex.main=2.8, outline=T, ylim=range(r1), main=plegend(p), col=c("lightblue", "lightgreen", "firebrick1"), axes=F, ylab="")
	axis(2, at=r1, labels=r0, las=1, font.axis=2.8, cex.axis=cex, lwd=5)
	mtext(mirnames[m], side=2, line=6, cex=2.8)
	mtext(c("Adult", "Senior", "Senior 75+"), side=1, line=3, at=seq(l), cex=2.8)
