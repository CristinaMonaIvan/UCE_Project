...
for (m in mir) {
	...
	l <- lapply(l, scale)
	tiff(paste(m,"MSSMSI_CRC.tiff", sep="_"), width=9*300, height=2700, res=300, compression="lzw")
	...
	mtext(c("MSS", "MSI"), side=1, line=3, at=seq(l), cex=2.8)
