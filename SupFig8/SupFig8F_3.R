...
x1 <- read.table("MIR4527HG.txt", sep="\t", head=T, quote="\"", check.names=F, stringsAsFactors=F)
x2 <- read.table("bed_filtered_merged_mutect_pindel_568_samples_201002_snpfiltered_ucr_CRC_CLL_miR4527HG.txt", sep="\t", head=T, quote="\"", check.names=F, stringsAsFactors=F)
...
x <- x[x1$gene_start[1] < x$POS & x$POS < x1$gene_end[1],]

x$TYPE <- gsub("CLL.*", "CLL", x$TYPE)
x$TYPE <- gsub("CRC.*", "CRC", x$TYPE)
...
s <- readBStringSet("USCSgenomebrowser4527HG.txt")
...
r <- rbind(get.region(seq, seq.start, "intron", "[a-z]+"),
		...
		get.region(seq, seq.start, "miR-4527", tolower(gsub("U", "T", "UGGUCUGCAAAGAGAUGACUGU"))))

u <- x[,c("REGION", "REGION_START", "REGION_END")]
u <- u[!duplicated(u),]
colnames(u) <- c("name", "start", "end")
col.region <- c(brewer.pal(12, "Set3"), brewer.pal(8, "Set1"), brewer.pal(8, "Dark2")[c(6,8,1:5,7)])[1:nrow(u)]
names(col.region) <- u$name

r$name <- as.character(r$name)
col.rect <- c("black", "black", "lightblue")
names(col.rect) <- unique(r$name)[c(3,2,1)]
pch.rect <- c(3,4,15)
...
height[top] <- jitter.vert2(pos[top], 2.3, 10000, .15)
height[!top] <-jitter.vert2(pos[!top], -.8, 10000, -.15)

col.mut <- rainbow(length(unique(mut)))
names(col.mut) <- unique(mut)

pch.type <- c(10:20, 23, 25)[1:length(unique(type))]
names(pch.type) <- sort(unique(type))

tiff("MIR4527HG.tiff", width=12*300, height=6*300, res=300, compression="lzw")
par(mar=c(3,5,6,9)+.1,font=2)

plot(NULL, xlim=range(pretty(c(r$start, r$end))), ylim=c(-1, 2.5), xlab="", ylab="", cex.main=3, axes=F)
axis(1, at=pretty(c(r$start, r$end)), lwd=3, cex.axis=1.2)
...
sym.height <- jitter.vert2(sym$mid, rect.upper+.08, 2000, -.1)
...
legend(xrel(.05), 3.7, legend=names(pch.type), pch=pch.type, 
		cex=1.2, bty="n", col="black", pt.bg="black", xpd=NA, ncol=3)
...
legend(xrel(.59), 3.7, 
		...
		pch=15, cex=1.2, bty="n", col=col.region, xpd=NA, ncol=4)
