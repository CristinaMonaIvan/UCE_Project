...
complement <- function (x) {
	...
	sapply(strsplit(as.character(x), NULL), function (y) paste(comp[y], collapse=""))
}
...
x1 <- read.table("MIR924HG.txt", sep="\t", head=T, quote="\"", check.names=F, stringsAsFactors=F)
x2 <- read.table("bed_filtered_merged_mutect_pindel_568_samples_201002_snpfiltered_ucr_CRC_CLL_miR924HG.txt", sep="\t", head=T, quote="\"", check.names=F, stringsAsFactors=F)

x <- rbind(data.frame(POS=x1$POS, REF=x1$REF, ALT=x1$ALT, TYPE=x1$type_pie, 
				REGION=x1$gene, REGION_START=x1$ucr_start, REGION_END=x1$ucr_end, TOP=T),
		data.frame(POS=x2$start, REF=x2$ref_allele, ALT=x2$alt_allele, TYPE=x2$DISEASE.TYPE,
				REGION=x2$region_name, REGION_START=x2$start.1, REGION_END=x2$end.1, TOP=F))

x <- x[x1$gene_start[1] <= x$POS & x$POS <= x1$gene_end[1],]
x <- x[order(x$POS),]

s <- readBStringSet("UCSCgenomebrowser924HG.txt")
...
r <- rbind(get.region(seq, seq.start, "intron", "[a-z]+"),
		...
		get.region(seq, seq.start, "hsa-miR-924", tolower(reverse(gsub("U", "T", "AGAGUCUUGUGAUGUCUUGC")))))
...
top <- x$TOP
pos <- x$POS
mut <- paste0(complement(x$REF), ">", complement(x$ALT))
...
tiff("MIR924HG_neg.tiff", width=12*300, height=6*300, res=300, compression="lzw")
par(mar=c(3,7,6,1)+.1,font=2)
...
sym.height <- jitter.vert2(sym$mid, rect.height-.01, 2000, -.1)
...
legend(xrel(.12), 2.55, legend=names(pch.type), pch=pch.type, 
