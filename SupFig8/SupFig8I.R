...
x <- read.table("ICGC_MDACC.txt", sep="\t", head=T, quote="\"", check.names=F, stringsAsFactors=F)
...
tiff("drug.tiff", width=3000, height=5700, res=300, compression="lzw")
...
axis(4, at=1:ncol(y), labels=colnames(y), las=1, lwd.ticks=0, font.axis=2, mgp=c(3,.5,0))
box()
