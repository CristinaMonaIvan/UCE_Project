...
x <- read.table("CRC_clinic.txt", sep="\t", head=T, quote="\"", check.names=F, na="")
...
for (m in mir) {
	...
	l <- lapply(l, scale)
	tiff(paste(m,"leftRightColorectal2.tiff", sep="_"), width=9*300, height=2700, res=300, compression="lzw")
	...
	mtext(c("Left", "Right"), side=1, line=3, at=seq(l), cex=2.8)
