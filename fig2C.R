...
setwd("C:\\Users\\CAntonescu\\Desktop\\fig2D")
...
x <- read.table("all.txt", sep="\t", head=T, quote="\"", fill=T, row.names=1)
...
barplot3d(rows=30,cols=22,z=t(M),scalexy=5, gap=0.3, alpha=0.2,theta=45,phi=50,
		topcolors=col, xlabels = F,ylabels=F,
		xsub="",ysub="",zsub="")
...
rgl.snapshot("plotNoWriting.png", fmt = "png", top = TRUE)


#erst alles bis barplot3d ausführen, dann Fenster maximieren/drehen, dann rgl.snapshot.
